...
(library (subdivide)
  ...
  ;;------------------------------------------------------------------
  ;; These identifiers have special meanings in our pattern language.
  (define ~) ;; rest
  (define event) ;; denotes a new sequence of adding
  
  ;;------------------------------------------------------------------
  ;; Allows definition of nested lists without (quasi)quoting.
  ;; Lists that start with an identifier are evaluated, unless the
  ;; identifier is the special character '~', which denotes a rest.
  ;; Lists that don't start with an identifier are just a list.
  ;;
  ;; (auto-quasi (1 2 "hi" (+ 2 2) (~ (+ 5 5)))) => (1 2 "hi" 4 (~ 10))
